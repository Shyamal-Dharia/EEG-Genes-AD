...
def shuffle_labels_in_loader(data_loader, batch_size):
    """Create a new DataLoader with shuffled labels"""
    ...
    shuffled_labels_tensor = torch.tensor(shuffled_labels)
    
    # Create new dataset and loader
    shuffled_dataset = TensorDataset(all_data, shuffled_labels_tensor)
    shuffled_loader = DataLoader(shuffled_dataset, batch_size=batch_size, shuffle=False)
...
# === NEW: Store trained models for permutation test ===
trained_models = {}
...
for subject in valid_subjects:
    ...
    # === NEW: Store trained model for permutation test ===
    trained_models[subject] = copy.deepcopy(model)
...
np.savez(
    'single_run_gradients_and_scores_0vs2.npz',
...
print("\nAll done! Saved results to 'single_run_gradients_and_scores_0vs2.npz'.")

# =====================================================================
#             NEW: EFFICIENT PERMUTATION TEST
# =====================================================================
...
observed_sample_f1 = sample_f1
observed_sample_auc = sample_roc_auc
observed_subject_f1 = subject_f1
observed_subject_auc = subject_roc_auc

print(f"Observed Sample F1 (macro): {observed_sample_f1:.4f}")
print(f"Observed Sample ROC AUC: {observed_sample_auc:.4f}")
print(f"Observed Subject F1 (macro): {observed_subject_f1:.4f}")
print(f"Observed Subject ROC AUC: {observed_subject_auc:.4f}")

# Permutation test parameters
n_permutations = 1000
destruction_epochs = 3
destruction_lr = 0.01

print(f"\nRunning {n_permutations} permutations with {destruction_epochs} destruction epochs...")
...
for perm_idx in range(n_permutations):
    ...
    # Set different seed for each permutation
    seed_everything(seed_value + perm_idx + 1000)
    
    # Storage for this permutation
    perm_sample_preds = []
    perm_sample_labels = []
    perm_sample_probs = []
    perm_subject_preds = []
    perm_subject_actuals = []
    perm_subject_scores = []
    
    # Process each subject with destruction training
    for subject in valid_subjects:
        if subject not in trained_models:
            continue
        ...
        model = copy.deepcopy(trained_models[subject]).to(device)
        
        # Create shuffled training data for destruction
        shuffled_train_loader = shuffle_labels_in_loader(train_loader, batch_size)
        
        # Destruction training
        destruction_optimizer = torch.optim.AdamW(model.parameters(), lr=destruction_lr, weight_decay=0.001)
        class_weights = torch.tensor([1.0, 1.0]).to(device)
        loss_fn = nn.CrossEntropyLoss(weight=class_weights)
        
        for epoch in range(destruction_epochs):
            train_loss, train_acc = train(model, shuffled_train_loader, loss_fn=loss_fn, 
                                        optimizer=destruction_optimizer, device=device)
        
        # Shuffle test labels too
        shuffled_test_loader = shuffle_labels_in_loader(test_loader, batch_size)
        ...
        with torch.no_grad():
            for X, y in shuffled_test_loader:
                X = X.to(device)
                outputs, wei = model(X)
                probs = torch.softmax(outputs, dim=1)
                preds = outputs.argmax(dim=1)
                ...
                subject_preds.append(preds.cpu())
                subject_probs.append(probs[:, 1].cpu())
                subject_labels.append(y.cpu())
        ...
        # Store predictions
        perm_sample_preds.extend(subject_preds.tolist())
        perm_sample_labels.extend(subject_labels.tolist())
        perm_sample_probs.extend(subject_probs.tolist())
        ...
        actual_class = subject_labels[0].item()
        perm_subject_preds.append(final_pred)
        perm_subject_actuals.append(actual_class)
        perm_subject_scores.append(majority_percentage)
    
    # Compute metrics for this permutation
    if len(perm_sample_preds) > 0:
        perm_sample_f1 = f1_score(perm_sample_labels, perm_sample_preds, average='macro')
        perm_sample_auc = roc_auc_score(perm_sample_labels, perm_sample_probs)
        perm_subject_f1 = f1_score(perm_subject_actuals, perm_subject_preds, average='macro')
        
        try:
            perm_subject_auc = roc_auc_score(perm_subject_actuals, perm_subject_scores)
        except ValueError:
            perm_subject_auc = np.nan
        
        null_sample_f1.append(perm_sample_f1)
        null_sample_auc.append(perm_sample_auc)
        null_subject_f1.append(perm_subject_f1)
        null_subject_auc.append(perm_subject_auc)

# Compute p-values
def compute_p_value(observed, null_dist):
    """Compute one-tailed p-value"""
    return (np.sum(np.array(null_dist) >= observed) + 1) / (len(null_dist) + 1)

p_value_sample_f1 = compute_p_value(observed_sample_f1, null_sample_f1)
p_value_sample_auc = compute_p_value(observed_sample_auc, null_sample_auc)
p_value_subject_f1 = compute_p_value(observed_subject_f1, null_subject_f1)
p_value_subject_auc = compute_p_value(observed_subject_auc, null_subject_auc)

# Print results
print(f"\n{'='*40}")
print(f"PERMUTATION TEST RESULTS (0 vs 2)")
print(f"{'='*40}")
print(f"Sample-level F1 (macro): {observed_sample_f1:.4f} (p = {p_value_sample_f1:.4f})")
print(f"Sample-level ROC AUC:    {observed_sample_auc:.4f} (p = {p_value_sample_auc:.4f})")
print(f"Subject-level F1 (macro): {observed_subject_f1:.4f} (p = {p_value_subject_f1:.4f})")
print(f"Subject-level ROC AUC:   {observed_subject_auc:.4f} (p = {p_value_subject_auc:.4f})")

significance_level = 0.05
print(f"\nSignificance at α = {significance_level}:")
print(f"Sample F1:  {'Significant' if p_value_sample_f1 < significance_level else 'Not significant'}")
print(f"Sample AUC: {'Significant' if p_value_sample_auc < significance_level else 'Not significant'}")
print(f"Subject F1: {'Significant' if p_value_subject_f1 < significance_level else 'Not significant'}")
print(f"Subject AUC: {'Significant' if p_value_subject_auc < significance_level else 'Not significant'}")

# Save permutation test results
np.savez(
    'permutation_test_results_0vs2.npz',
    observed_sample_f1=observed_sample_f1,
    observed_sample_auc=observed_sample_auc,
    observed_subject_f1=observed_subject_f1,
    observed_subject_auc=observed_subject_auc,
    null_sample_f1=null_sample_f1,
    null_sample_auc=null_sample_auc,
    null_subject_f1=null_subject_f1,
    null_subject_auc=null_subject_auc,
    p_value_sample_f1=p_value_sample_f1,
    p_value_sample_auc=p_value_sample_auc,
    p_value_subject_f1=p_value_subject_f1,
    p_value_subject_auc=p_value_subject_auc,
    n_permutations=n_permutations,
    destruction_epochs=destruction_epochs
)

print(f"\nPermutation test results saved to 'permutation_test_results_0vs2.npz'")
print("All done!")