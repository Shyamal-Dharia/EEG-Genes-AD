...
csv_data = pd.read_csv('/home/s.dharia-ra/Shyamal/PEARL_EEG/filtered_participants.tsv', sep='\t')

skipped_log = []
fs = 256
id_pattern = re.compile(r'(sub-\d+)')
npz_files = glob.glob('/home/s.dharia-ra/Shyamal/PEARL_EEG_NEW/resting_state_preprocessed_data/*.npz')

# Create an output directory for the features.
output_dir = 'HFD_PSD_stats_features_40sec'
...
for npz_file in npz_files:
    ...
    # Define window size (e.g., 40-second windows; adjust if necessary).
    window_size = 40 * fs  # samples per window
...
if skipped_log:
    with open("skipped_log.txt", "w") as log_file:
        log_file.write("\n".join(skipped_log))
